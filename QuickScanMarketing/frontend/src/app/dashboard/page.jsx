    ...
    <UserAuthWrapper>
      ...
      <div className="flex-1 flex flex-col min-w-0 h-full overflow-y-auto relative">
        <header className="h-16 border-b border-white/5 flex items-center justify-between px-6 bg-[#0B132B]/90 backdrop-blur-xl sticky top-0 z-40">
          ...
          <div className="flex items-center gap-4">
            <button onClick={() => setIsNotificationPanelOpen(!isNotificationPanelOpen)} className={`relative w-10 h-10 rounded-xl border flex items-center justify-center transition-all ${isNotificationPanelOpen ? 'bg-blue-500/10 border-blue-500/30 text-blue-400' : 'bg-white/5 border-white/10 text-slate-400'}`}>
              <Bell size={20} className={notifications.some(n => !n.read) ? 'animate-bounce' : ''} />
              {notifications.some(n => !n.read) && <span className="absolute -top-1 -right-1 min-w-[18px] h-[18px] bg-blue-600 rounded-full border-2 border-[#0B132B] text-[8px] font-black flex items-center justify-center px-1">{notifications.filter(n => !n.read).length}</span>}
            </button>
            <div className="w-8 h-8 rounded bg-linear-to-br from-purple-600 to-indigo-600 flex items-center justify-center shadow-lg"><User size={16} className="text-white" /></div>
            <span className="hidden sm:block text-xs font-black text-white/80">{userName}</span>
            <button onClick={handleLogout} className="w-9 h-9 rounded-xl border border-white/10 bg-white/5 hover:text-rose-400 transition-all"><LogOut size={16} /></button>
          </div>
        </header>
        ...
        {view === 'overview' && (
          <>
            <section>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">{stats.map((stat) => (<div key={stat.name} className="bg-[#1F2B4B] border border-white/5 p-5 rounded-sm relative group transition-colors"><div className="flex justify-between items-start mb-2"><div className="space-y-1"><p className="text-[9px] font-black text-slate-500 uppercase tracking-wider">{stat.name}</p><h3 className="text-3xl font-black italic">{stat.value}</h3></div><stat.icon size={16} className={`${stat.color} opacity-40 group-hover:opacity-100`} /></div><div className="h-[2px] w-full bg-white/5 mt-4"><div className={`h-full bg-current ${stat.color} w-1/3 shadow-[0_0_10px_rgba(0,0,0,0.5)]`}></div></div></div>))}</div>
            </section>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                qrList.map((qr) => (
                  <div key={qr._id} className="bg-[#1F2B4B] border border-white/5 rounded-xl p-5 hover:border-blue-500/50 hover:shadow-[0_0_15px_rgba(59,130,246,0.15)] transition-all group flex flex-col relative overflow-hidden">
                    ...
                    <div className="flex justify-center mb-4 mt-2"><div className="bg-white p-3 rounded-xl transition-all shadow-[0_0_15px_rgba(255,255,255,0.1)] group-hover:scale-105"><QRCode value={`${apiUrl}/qr/scan/${qr._id}`} size={140} style={{ height: "auto", maxWidth: "100%", width: "100%" }} /></div></div>
