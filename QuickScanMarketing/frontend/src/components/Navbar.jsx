...
import { EyeOff, Eye, Menu, X, QrCode } from 'lucide-react'
      ...
      <div className="mx-auto max-w-screen-2xl px-4 md:px-8">
        <header className="flex items-center justify-between py-4">
          ...
          <div className="hidden flex-col gap-2.5 sm:flex-row sm:justify-center lg:flex lg:justify-start items-center">
            <Link href="/login" className="inline-block px-4 py-2.5 text-center text-sm font-bold text-blue-100/70 hover:bg-blue-600/20 hover:text-blue-300 rounded-full transition uppercase tracking-wider">
              LOGIN
            </Link>
            <Link href="/signup" className="inline-block rounded-full bg-blue-600 px-6 py-2.5 text-center text-sm font-bold text-white shadow-[0_0_15px_rgba(37,99,235,0.4)] hover:bg-blue-500 hover:shadow-[0_0_20px_rgba(37,99,235,0.6)] transition-all transform hover:-translate-y-0.5">
              Sign up
            </Link>
        ...
        {isMobileMenuOpen && (
           <div className="lg:hidden absolute top-full left-0 w-full bg-[#0A1128] shadow-2xl border-b border-white/5 py-4 px-4 flex flex-col gap-4 animate-in slide-in-from-top-2">
             ...
             <Link href="/login" onClick={() => setIsMobileMenuOpen(false)} className="block px-4 py-2 text-base font-bold text-blue-100/80 hover:text-blue-400 hover:bg-blue-500/10 rounded-lg">Login</Link>
             <Link href="/signup" onClick={() => setIsMobileMenuOpen(false)} className="block px-4 py-2 text-base font-bold text-blue-100/80 hover:text-blue-400 hover:bg-blue-500/10 rounded-lg">Signup</Link>
             <hr className="border-white/5 my-2" />
             <div className="flex flex-col gap-3">
                <Link href="/login" onClick={() => setIsMobileMenuOpen(false)} className="block text-center px-4 py-2 text-sm font-bold text-blue-100/80 border border-white/10 rounded-lg hover:bg-blue-500/20 hover:text-blue-300 hover:border-blue-500/30 transition-colors uppercase tracking-wider">LOGIN</Link>
                <Link href="/signup" onClick={() => setIsMobileMenuOpen(false)} className="block text-center px-4 py-2 text-sm font-bold text-white bg-blue-600 rounded-lg shadow-[0_0_15px_rgba(37,99,235,0.3)] hover:bg-blue-500">Sign up</Link>
             </div>
           </div>
        )}
      </div>
